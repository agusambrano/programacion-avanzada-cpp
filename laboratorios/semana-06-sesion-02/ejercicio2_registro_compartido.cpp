...
class MonitorDeVuelo {
    ...
    public:
        ...
        MonitorDeVuelo(int unId, std::shared_ptr<RegistroDeVuelo> unRegistro) {
            idMonitor = unId;
            registro = std::move(unRegistro);
        }

        void reportar(int indice) {
            std::cout << "Monitor " << idMonitor << " ve altura " << registro->getAltura(indice) << std::endl;
        }
...
int main() {
    ...
    std::cout << "use_count: " << compartido.use_count() << std::endl;

    {
        MonitorDeVuelo torre(1, compartido);
        MonitorDeVuelo cabina(2, compartido);
